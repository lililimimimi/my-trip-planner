...
注意：坐标必须真实准确，餐厅必须在景点附近，温度纯数字。local_foods必须根据目的地城市推荐5-6个最具代表性的当地特色小吃，不要具体店名，只介绍食物本身。"""
...
class TripAgent:
    ...
    def chat(self, user_message: str, replan_mode: str = None) -> dict:
        ...
        if city:
            ...
            if real_attractions:
                context += f"\n【真实景点数据，坐标100%准确，必须从中选择景点，不能自己编造】：{json.dumps(real_attractions, ensure_ascii=False)}"
            if real_restaurants:
                context += f"\n【真实餐厅数据，坐标100%准确，必须从中选择餐厅，不能自己编造】：{json.dumps(real_restaurants, ensure_ascii=False)}"
            if weather:
