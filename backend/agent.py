...
from tools import search_attractions, search_restaurants, search_hotels, get_weather, search_knowledge_base, get_place_detail, search_nearby_restaurants, get_city_attractions, get_city_restaurants, get_local_foods, search_rag
...
如果有【旅行攻略参考】数据，景点推荐必须优先参考攻略里提到的景点，不要自己编造没有在攻略里出现过的景点。
...
class TripAgent:
    ...
    def chat(self, user_message: str, replan_mode: str = None) -> dict:
        ...
        if city:
            ...
            # RAG 搜索旅行攻略
            rag_result = search_rag(user_message, city)
            if rag_result:
                context += f"\n【旅行攻略参考】：{rag_result}"
